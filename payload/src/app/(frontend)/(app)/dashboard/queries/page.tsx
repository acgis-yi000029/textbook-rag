...
export default function Page() {
  ...
  const fetchTemplates = useCallback(async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/query-templates?limit=50&sort=sortOrder')
  ...
  const handleSave = async () => {
    ...
    setSaved(false)
    try {
      const url = isNew ? '/api/query-templates' : `/api/query-templates/${selected!.id}`
  ...
  const handleDelete = async () => {
    ...
    await fetch(`/api/query-templates/${selected.id}`, { method: 'DELETE', credentials: 'include' })
    setSelected(null)
    setDraft({})
    await fetchTemplates()
  }
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-foreground">
              {isZh ? '问题引导模板' : 'Query Templates'}
            </span>
          </div>
          <button onClick={startNew} className="p-1.5 rounded-md text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors">
