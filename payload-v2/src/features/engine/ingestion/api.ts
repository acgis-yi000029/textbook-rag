/**
 * engine/ingestion/api.ts
 * Aligned with: llama_index.ingestion → engine-v2/ingestion/ → collections/IngestTasks
 *
 * Pipeline operations: trigger ingest, rebuild vectors, sync to Payload.
 */

import type { PipelineTask, TriggerPipelineRequest, SyncResult, PipelinePreview } from './types'
...
export async function triggerPipeline(req: TriggerPipelineRequest): Promise<PipelineTask> {
  ...
  const taskResp = await fetch('/api/ingest-tasks', {
    method: 'POST',
  ...
  if (!engineResp.ok) {
    ...
    await fetch(`/api/ingest-tasks/${task.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
...
export async function fetchBookTasks(bookId: number): Promise<PipelineTask[]> {
  const resp = await fetch(
    `/api/ingest-tasks?where[book][equals]=${bookId}&sort=-createdAt&limit=10`,
    { credentials: 'include' }
  )
  if (!resp.ok) return []
...
export async function fetchTask(taskId: number): Promise<PipelineTask | null> {
  try {
    const resp = await fetch(`/api/ingest-tasks/${taskId}`, { credentials: 'include' })
...
/** Trigger a full sync from Engine → Payload CMS */
...
/** Re-index all books (rebuild vectors) */
...
/** Fetch pipeline stage preview for a book (calls Engine directly) */
export async function fetchPipelinePreview(engineBookId: string): Promise<PipelinePreview> {
  return request<PipelinePreview>(
    `${ENGINE}/engine/pipeline-preview/${encodeURIComponent(engineBookId)}`,
  )
}
