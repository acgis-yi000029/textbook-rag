...
- This repository does not use cloud deployment, container orchestration, or CI/CD as the required deployment target for the `/dev` workflow.
- The deployment phase for this repo is satisfied by local full-stack startup through `.vscode` configuration.
- Prefer maintaining `Start Full` and `Start Full Stack` launch/task entries that run the frontend and backend locally.
- Treat Docker, Kubernetes, Terraform, and hosted pipeline examples in this skill as optional references only.
