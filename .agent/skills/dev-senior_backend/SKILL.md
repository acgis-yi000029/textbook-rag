...
1. Before changing backend code for a substantial task, first create or update a backend codemap that maps stories to modules, files, interfaces, and data flow.
2. Treat the codemap as the implementation contract for the backend phase; coding starts only after the target module boundaries are explicit.
3. Keep the codemap synchronized when the implementation plan changes materially during the phase.
