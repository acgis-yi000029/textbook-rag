...
- **产出物**: `docs/codemaps/testing.md`, `backend/tests/`, `docs/test-report.md`
...
- `docs/codemaps/backend.md` - 后端实现映射
- `docs/codemaps/frontend.md` - 前端实现映射
- `backend/app/` - 后端代码
- `frontend/src/` - 前端代码

### 1.1 先产出 Testing Codemap

在写任何测试之前，先创建或更新 `docs/codemaps/testing.md`，明确：

- Story 到测试用例的映射
- 单元、集成、手工验证的边界
- 关键风险路径和必测路径
- 后端与前端分别需要的验证方式
- 哪些测试是 phase gate 的阻塞项

只有在 testing codemap 明确之后，才进入测试实现。
...
**在写测试之前，并且在 testing codemap 已完成之后，先执行以下步骤：**
...
- [ ] 单元测试覆盖率达标
- [ ] `docs/codemaps/testing.md` 已创建并与测试实现一致
- [ ] 集成测试通过
- [ ] E2E 测试通过
- [ ] 无阻断性 Bug
- [ ] 测试报告已生成
